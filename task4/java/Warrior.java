...
public class Warrior{
	...
	public boolean actionOnWarrior(Warrior warrior) {
		this.talk("Hi, bro. You can call me "+ this.getName() +".  I am very happy to meet you.  " + String.format("I have %d magic crystals.", this.getMagic_crystal()));
		...
		if (a == 1) {
			int value = TheJourney.rand.nextInt(this.getMagic_crystal()-2)+2;
			if (this.getMagic_crystal() > value) {
				this.decreaseCrystal(value);
				warrior.increaseCrystal(value);
				warrior.talk(String.format("Thanks for your shared %d magic crystals !", value)+ this.getName()+".");
			} else {
				this.talk("Very embarrassing, I don't have enough crystals.");
				warrior.talk("That is fine, " + this.getName() + " ! After all we are all poor sons.");
			}
		}
		return false;
	}
